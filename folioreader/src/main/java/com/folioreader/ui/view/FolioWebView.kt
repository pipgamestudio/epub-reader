...
import com.folioreader.util.Dict
...
class FolioWebView : WebView {
    ...
    fun onTextSelectionItemClicked(id: Int, selectedText: String?) {
        ...
        when (id) {
            ...
            R.id.translateSelection -> {
                ...
                val b = Charset.forName("US-ASCII").newEncoder().canEncode(selectedText)
                if (b && !selectedText!!.contains(" "))  showTranslateDialog(selectedText)
                else Toast.makeText(context, "不是英文單字，翻譯不能。", Toast.LENGTH_SHORT).show()
            }
            else -> {
    ...
    private fun showTranslateDialog(selectedText: String?) {
        ...
        val txt = Dict.getInstance(context).lookup(selectedText)
